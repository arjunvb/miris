...
def zip_frame_info(detections, frame_idx):
    ...
    im_bounds = geom.Rectangle(geom.Point(0, 0), geom.Point(im.shape[0], im.shape[1]))
    info = []
    for idx, detection in enumerate(detections):
        rect = geom.Rectangle(
            geom.Point(detection["top"] / frame_scale, detection["left"] / frame_scale),
            geom.Point(
                detection["bottom"] / frame_scale, detection["right"] / frame_scale
            ),
        )
        ...
        resize_factor = min(
            [float(CROP_SIZE) / crop.shape[0], float(CROP_SIZE) / crop.shape[1]]
        )
        crop = (
        ...
        detection["height"] = (
            float(detection["bottom"] - detection["top"]) / ORIG_HEIGHT
        )
...
def get_frame_pair(info1, info2, skip):
    ...
    input_nodes = []
    input_edges = []
    input_crops = numpy.zeros(
        (len(info1) + 1 + len(info2), CROP_SIZE, CROP_SIZE, 3), dtype="uint8"
    )
    ...
    for i, t in enumerate(info1):
        ...
        input_nodes.append(
            [cx, cy, detection["width"], detection["height"], 1, 0, 0, skip / 50.0]
            + [0.0] * 64
        )
        input_crops[i, :, :, :] = crop
    ...
    for i, t in enumerate(info2):
        ...
        cx, cy = get_loc(detection)
        input_nodes.append(
            [cx, cy, detection["width"], detection["height"], 0, 0, 1, skip / 50.0]
            + [0.0] * 64
        )
...
while True:
    ...
    for idx1, idx2 in indices:
        ...
        if len(info1) == 0 or len(info2) == 0:
            mats.append(
                numpy.zeros((len(info1), len(info2) + 1), dtype="float32").tolist()
            )
            continue

        input_dict, input_crops = get_frame_pair(info1, info2, idx2 - idx1)
        d1 = graph_nets.utils_tf.get_feed_dict(
            m.inputs, graph_nets.utils_np.data_dicts_to_graphs_tuple([input_dict])
        )
        feed_dict = {
        ...
        mat = numpy.zeros(
            (len(detections[idx1]), len(detections[idx2]) + 1), dtype="float32"
        )
